...
ALARMINFO_V30_ALARMTYPE_RECORDING_RESOLUTION_DOES_NOT_MATCH_WITH_WHICH_OF_FRONT_END_CAMERA = (
    13
)
...
def setupSDK():
    ...
    """
    logger.info(
        f"Using OS: {platform.uname()[0]} with architecture: {platform.uname()[4]}"
    )
...
cbLoginResult = CFUNCTYPE(c_void_p, LONG, DWORD, LPNET_DVR_DEVICE_INFO, c_void_p)
...
class NET_DVR_NOTICEDATA_RECEIPT_INFO(Structure):
    _fields_ = [("byNoticeNumber", BYTE * MAX_NOTICE_NUMBER_LEN), ("byRes", BYTE * 224)]


class NET_DVR_SEND_CARD_INFO(Structure):
    _fields_ = [("byCardNo", BYTE * ACS_CARD_NO_LEN), ("byRes", BYTE * 224)]
