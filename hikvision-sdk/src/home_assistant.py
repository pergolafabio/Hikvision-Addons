...
class HomeAssistantAPI(EventHandler):
    ...
    def __init__(self, config) -> None:
        super().__init__()
        self.sensors['door'] = "sensor." + config.sensor_door
        self.sensors['callstatus'] = "sensor." + config.sensor_callstatus
        self.sensors['motion'] = "sensor." + config.sensor_motion
        self.sensors['tamper'] = "sensor." + config.sensor_tamper
        self.sensors['dismiss'] = "sensor." + config.sensor_dismiss
    ...
    async def motion_detection(self, command: int, device: NET_DVR_ALARMER, alarm_info: NET_DVR_ALARMINFO_V30, buffer_length, user_pointer: c_void_p):
        logger.info("Motion detected, updating sensor {}", self.sensors['motion'])
        update_sensor(self.sensors['motion'], 'on')
        await asyncio.sleep(1)
        update_sensor(self.sensors['motion'], 'off')
    ...
    async def video_intercom_event(self, command: int, device: NET_DVR_ALARMER, alarm_info: NET_DVR_VIDEO_INTERCOM_EVENT, buffer_length, user_pointer: c_void_p):
        if alarm_info.byEventType == VIDEO_INTERCOM_EVENT_EVENTTYPE_UNLOCK_LOG:
            ...
            update_sensor(self.sensors['door'], 'on', attributes)
            await asyncio.sleep(1)
            update_sensor(self.sensors['door'], 'off', attributes)
    ...
    async def video_intercom_alarm(self, command: int, device: NET_DVR_ALARMER, alarm_info: NET_DVR_VIDEO_INTERCOM_ALARM, buffer_length, user_pointer: c_void_p):
        if alarm_info.byAlarmType == VIDEO_INTERCOM_ALARM_ALARMTYPE_DOORBELL_RINGING:
            logger.info("Doorbell ringing, updating sensor {}", self.sensors['callstatus'])
            update_sensor(self.sensors['callstatus'], 'on')
            await asyncio.sleep(1)
            update_sensor(self.sensors['callstatus'], 'off')
        elif alarm_info.byAlarmType == VIDEO_INTERCOM_ALARM_ALARMTYPE_DISMISS_INCOMING_CALL:
            logger.info("Call dismissed, updating sensor {}", self.sensors['dismiss'])
            update_sensor(self.sensors['dismiss'], 'on')
            await asyncio.sleep(1)
            update_sensor(self.sensors['dismiss'], 'off')
        elif alarm_info.byAlarmType == VIDEO_INTERCOM_ALARM_ALARMTYPE_TAMPERING_ALARM:
            logger.info("Tampering alarm, updating sensor {}", self.sensors['tamper'])
            update_sensor(self.sensors['tamper'], 'on')
            await asyncio.sleep(1)
            update_sensor(self.sensors['tamper'], 'off')
...
HA_BASE_URL = "http://supervisor"
HTTP_HEADERS = {
    'Authorization': f'Bearer {SUPERVISOR_TOKEN}'
}


def update_sensor(sensor_name: str, state: str, attr: dict = {}):
    """ Update the sensor by invoking the Home Assistant HTTP API
    """

    data = {'state': state, 'attributes': attr}
    try:
        response = requests.post(f"{HA_BASE_URL}/core/api/states/{sensor_name}", headers=HTTP_HEADERS, json=data)
